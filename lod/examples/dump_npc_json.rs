...
fn main() {
    ...
    for (i, a) in ddm.actors.iter().enumerate() {
        ...
        let (display_name, portrait, npc_type) = if a.npc_id > 0 && !is_peasant {
            ...
            let name = npc_table.npc_name(a.npc_id as i32)
                .unwrap_or(&a.name)
                .to_string();
            let portrait = npc_table.portrait_name(a.npc_id as i32)
                .unwrap_or_else(|| "none".into());
            (name, portrait, "quest")
        } else if is_peasant {
            // Peasant NPC: identity from npcdata.txt peasant-profession entries
            let (name, portrait_num) = npc_table.peasant_identity(is_female, i)
                .unwrap_or(("Peasant", 1));
      ...
      "position": [{}, {}, {}]
            ...
            a.npc_id,
            if is_peasant {
                if a.npc_id == 1 { "type_flag: female peasant" }
                else { "type_flag: male peasant" }
            } else if a.npc_id > 0 {
                "npcdata.txt index"
            } else {
                "none (monster)"
