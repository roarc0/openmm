...
fn main() {
    ...
    for src_path in &lod_files {
        ...
        match lod::LodManager::new(lod_path) {
            Ok(mgr) => {
        ...
        let orig_mgr = match lod::LodManager::new(lod_path) {
            Ok(m) => m,
            Err(e) => {
        ...
        let written_mgr = match lod::LodManager::new(tmp_dir.as_path()) {
            Ok(m) => m,
            Err(_) => {
