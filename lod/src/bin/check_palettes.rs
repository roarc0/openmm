fn main() {
    ...
    for offset in (0..data.len().saturating_sub(40)).step_by(2) {
        ...
        if val > 0 && val < 500 && offset + 4 + val * 548 <= data.len() {
            if data[offset + 4] >= b'A' && data[offset + 4] <= b'Z' {
                actor_start = offset;
                break;
            }
        }
    ...
    for off in [0x54u16, 0x56, 0x58, 0x5A, 0x5C, 0x5E, 0x60, 0x62, 0x64, 0x66, 0x68, 0x6A, 0x6C, 0x6E, 0x70, 0x72, 0x74, 0x76] {
        ...
        if in_range && varies {
            let names: Vec<&str> = vals.iter().map(|v| 
                monlist.get(*v as usize).map(|m| m.internal_name.as_str()).unwrap_or("???")
            ).collect();
