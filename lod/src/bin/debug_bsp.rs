...
fn main() {
    ...
    for (mi, model) in map.bsp_models.iter().enumerate().take(5) {
        ...
        println!("  texture_names[{}]: {:?}", model.texture_names.len(), model.texture_names);
        ...
        for (fi, face) in model.faces.iter().enumerate().take(4) {
            ...
            println!("  Face {}: tex_id={}, verts={}, tex_u={}, tex_v={}",
