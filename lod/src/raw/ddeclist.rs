...
use crate::{LodManager, raw::enums::DecorationDescFlags, raw::lod_data::LodData, utils::try_read_name};
...
impl DDecList {
    pub fn load(lod_manager: &LodManager) -> Result<Self, Box<dyn Error>> {
        let raw = lod_manager.try_get_bytes("icons/ddeclist.bin")?;
...
mod tests {
    ...
    fn read_declist_data_works() {
        ...
        let d_declist = DDecList::load(&lod_manager).unwrap();
