...
fn parse_new_sorpigal_actors() {
    ...
    let ddm = Ddm::load(&lod, "oute3.odm").unwrap();
...
fn actor_monlist_id_is_zero_indexed() {
    ...
    let ddm = Ddm::load(&lod, "oute3.odm").unwrap();
    let monlist = crate::raw::monlist::MonsterList::load(&lod).unwrap();
    for actor in &ddm.actors {
        assert!(
...
fn parse_multiple_maps() {
    ...
    for map in &["oute3.odm", "oute2.odm"] {
        if let Ok(ddm) = Ddm::load(&lod, map) {
...
fn oute3_ddm_actors_near_goblin_spawn() {
    ...
    let ml = crate::raw::monlist::MonsterList::load(&lod).unwrap();
    let ddm = Ddm::load(&lod, "oute3.odm").unwrap();
