...
use crate::raw::{billboard::{Billboard, BillboardManager}, blv::BlvDecoration};
...
impl Decorations {
    ...
    pub fn load(lod: &LodManager, odm_billboards: &[Billboard]) -> Result<Self, Box<dyn Error>> {
        let mgr = BillboardManager::load(lod)?;
