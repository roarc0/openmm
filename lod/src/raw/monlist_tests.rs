...
fn ghost_walking_and_standing_palette_match() {
    ...
    let gd = crate::raw::global::GameData::load(&lod).unwrap();
    for dif in 1..=3u8 {
        let desc = gd
            .monlist
        ...
        let st = crate::raw::monster::resolve_sprite_group(st_group, &gd.dsft, &lod)
            .unwrap_or_else(|| panic!("Ghost {} standing group '{}' should resolve", dif, st_group));
        let wa = crate::raw::monster::resolve_sprite_group(wa_group, &gd.dsft, &lod)
