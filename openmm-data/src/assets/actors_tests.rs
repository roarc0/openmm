...
fn actors_loads_oute3() {
    ...
    let actors = Actors::new(&assets, "oute3.odm", None, &gd).unwrap();
...
fn state_snapshot_empty_filters_nothing() {
    ...
    let gd = game_data(&assets);
    let actors_all = Actors::new(&assets, "oute3.odm", None, &gd).unwrap();
    let all_count = actors_all.get_actors().len();
    let snapshot = MapStateSnapshot { dead_actor_ids: vec![] };
    let actors_with_state = Actors::new(&assets, "oute3.odm", Some(&snapshot), &gd).unwrap();
    assert_eq!(
...
/// Outdoor encounter monsters come exclusively from ODM spawn points via Monsters::load().
