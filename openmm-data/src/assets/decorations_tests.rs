...
fn load_oute3_decorations() -> Option<Decorations> {
    let assets = test_lod()?;
    let odm = crate::assets::odm::Odm::load(&assets, "oute3.odm").ok()?;
    Decorations::load(&assets, &odm.billboards).ok()
}
...
fn d01_torches_have_light_radius() {
    ...
    let blv = match Blv::load(&assets, "d01.blv") {
        Ok(b) => b,
        Err(_) => return,
    };
    let decs = Decorations::from_blv(&assets, &blv.decorations).unwrap();
    ...
    for t in &torches {
        assert!(
            t.light_radius > 0,
            "torch '{}' should have light_radius > 0",
            t.sprite_name
        );
    }
}
...
fn campfire_has_zero_light_radius_but_exists_in_blv_lights() {
    ...
    let blv = match Blv::load(&assets, "zddb01.blv") {
        Ok(b) => b,
        Err(_) => return,
    };
    let decs = Decorations::from_blv(&assets, &blv.decorations).unwrap();
    ...
    for c in &campfires {
        assert_eq!(
            c.light_radius, 0,
            "campfireon has no decoration light_radius (uses BLV lights instead)"
        );
    }
    // The BLV lights list is non-empty — those provide the campfire illumination.
    assert!(
        !blv.lights.is_empty(),
        "zddb01 should have BLV static lights for campfire illumination"
    );
    assert!(
        blv.lights.iter().any(|l| l.brightness >= 640),
        "at least one bright light (campfire) expected"
    );
}
