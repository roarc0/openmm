...
impl Dtile {
    pub fn load(assets: &Assets) -> Result<Self, Box<dyn Error>> {
        let raw = assets.get_bytes("icons/dtile.bin")?;
...
impl LodSerialise for Dtile {
    fn to_bytes(&self) -> Vec<u8> {
        use byteorder::{LittleEndian, WriteBytesExt};
        let mut buf: Vec<u8> = Vec::with_capacity(4 + self.tiles.len() * 26);
        buf.write_u32::<LittleEndian>(self.tiles.len() as u32).unwrap();
        for t in &self.tiles {
            buf.extend_from_slice(&t.name);
            buf.write_i16::<LittleEndian>(t.id).unwrap();
            buf.write_i16::<LittleEndian>(t.bitmap).unwrap();
            buf.write_i16::<LittleEndian>(t.tile_set).unwrap();
            buf.write_i16::<LittleEndian>(t.section).unwrap();
            buf.write_u16::<LittleEndian>(t.attributes.bits()).unwrap();
        }
        buf
    }
}
...
pub struct TileTable {
    size: (u8, u8),
    names_table: Vec<String>,
    names_set: Vec<String>,
    coordinates_table: Vec<(u8, u8)>,
}
...
impl TileTable {
    pub fn new(names_table: [String; 256]) -> Self {
        ...
        let mut t = TileTable {
            ...
            names_set,
            names_table: names_table.to_vec(),
            coordinates_table: vec![(0, 0); 256],
        };
    ...
    pub fn atlas_image(&self, assets: &Assets) -> Result<DynamicImage, Box<dyn Error>> {
        let ts: Vec<&str> = self.names_set.iter().map(|s| s.as_str()).collect();
        get_atlas(assets, ts.as_slice(), self.size.0 as usize)
    }
}
