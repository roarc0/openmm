...
impl LodSerialise for Odm {
    fn to_bytes(&self) -> Vec<u8> {
        ...
        use crate::assets::billboard::BillboardData;
...
fn write_bsp_model_header(buf: &mut Vec<u8>, h: &crate::assets::bsp_model::BSPModelHeader) {
    use byteorder::{LittleEndian, WriteBytesExt};
...
fn write_bsp_model_data(buf: &mut Vec<u8>, m: &crate::assets::bsp_model::BSPModel) {
    use byteorder::{LittleEndian, WriteBytesExt};
    for v in &m.vertices {
    ...
    for face in &m.faces {
        let face_bytes = unsafe {
            std::slice::from_raw_parts(
                face as *const crate::assets::bsp_model::BSPModelFace as *const u8,
                std::mem::size_of::<crate::assets::bsp_model::BSPModelFace>(),
            )
        };
...
impl Odm {
    ...
    pub fn tile_table(&self, assets: &Assets) -> Result<TileTable, Box<dyn Error>> {
        Dtile::load(assets)?
...
impl OdmData {
    ...
    fn tile_uvs(tile_table: &TileTable, tile_index: u8) -> ([f32; 2], [f32; 2], [f32; 2], [f32; 2]) {
        ...
        let pad = crate::assets::image::ATLAS_TILE_PAD as f32;
