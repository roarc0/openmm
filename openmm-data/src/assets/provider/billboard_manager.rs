//! BillboardManager — compositor that combines ddeclist.bin + dsft.bin to resolve
//! decoration sprites for rendering. Extracted from billboard.rs (pure data) because
//! this type is a compositor: it loads multiple assets and calls into GameLod/LodDecoder.
...
impl BillboardManager {
    pub fn load(assets: &Assets) -> Result<Self, Box<dyn std::error::Error>> {
