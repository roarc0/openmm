...
fn game_data(assets: &Assets) -> GameData {
    GameData::load(assets).expect("GameData::new failed")
}
...
fn monsters_loads_oute3() {
    ...
    let monsters = Monsters::load(&assets, "oute3.odm", &gd).unwrap();
...
fn goblin_a_resolves_standing_sprite() {
    ...
    let gd = game_data(&assets);
    let entry = resolve_entry(0, &gd, &assets);
...
fn resolve_sprite_group_goblin_standing() {
    ...
    let result = resolve_sprite_group(group, &gd.dsft, &assets);
...
fn resolve_entry_peasant_male_is_flagged() {
    ...
    let entry = resolve_entry(132, &gd, &assets);
...
fn resolve_entry_peasant_female_is_flagged() {
    ...
    let entry = resolve_entry(120, &gd, &assets);
...
fn oute3_forced_variant_spawns_produce_one_monster() {
    ...
    let gd = game_data(&assets);
    let odm = crate::assets::odm::Odm::load(&assets, "oute3.odm").unwrap();
    let monsters = Monsters::load(&assets, "oute3.odm", &gd).unwrap();
