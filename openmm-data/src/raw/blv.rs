...
use crate::raw::{enums::{DoorAttributes, FaceAttributes, PolygonType}, lod_data::LodData, odm::mm6_to_bevy};
...
mod tests {
    use super::*;
    use crate::{LodManager, get_data_path};

    #[test]
    fn parse_d01_blv() {
        let lod_manager = LodManager::new(get_data_path()).unwrap();
        ...
        let ddeclist = crate::raw::ddeclist::DDecList::load(&lod_manager).unwrap();
    ...
    fn parse_sewer_blv() {
        let lod_manager = LodManager::new(get_data_path()).unwrap();
        let blv = Blv::load(&lod_manager, "sewer.blv").unwrap();
    ...
    fn initialize_doors_d01() {
        let lod_manager = LodManager::new(get_data_path()).unwrap();
        let blv = Blv::load(&lod_manager, "d01.blv").unwrap();
        let dlv = crate::raw::dlv::Dlv::new(&lod_manager, "d01.blv", blv.door_count, blv.doors_data_size).unwrap();
