...
fn read_dtile_data_works() {
    ...
    let dtile = Dtile::load(&lod_manager).unwrap();
...
fn tileset_lookup_grass_map() {
    ...
    let odm = Odm::load(&lod_manager, "oute3.odm").unwrap();
    let dtile = Dtile::load(&lod_manager).unwrap();
...
fn tileset_lookup_snow_map() {
    ...
    let odm = Odm::load(&lod_manager, "outc1.odm").unwrap();
    let dtile = Dtile::load(&lod_manager).unwrap();
