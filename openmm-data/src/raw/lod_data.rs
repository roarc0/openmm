...
fn decompress_with_48_bytes_header(data: &[u8]) -> Result<Vec<u8>, Box<dyn Error>> {
    ...
    crate::raw::zlib::decompress(&data[48..], compressed_size, uncompressed_size)
}
...
fn decompress_with_8_bytes_header(data: &[u8]) -> Result<Vec<u8>, Box<dyn Error>> {
    ...
    crate::raw::zlib::decompress(&data[8..], compressed_size, decompressed_size)
}
    ...
    pub fn pack(&self) -> Vec<u8> {
        match self.kind {
            ...
            CompressionKind::Zlib8 => {
                let compressed = crate::raw::zlib::compress(&self.data);
