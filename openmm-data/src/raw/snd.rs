...
impl SndArchive {
    ...
    pub fn get(&self, name: &str) -> Option<Vec<u8>> {
        ...
        let wav = if entry.decompressed_size > 0 {
            crate::raw::zlib::decompress(raw, entry.size as usize, entry.decompressed_size as usize).ok()?
        } else {
            raw.to_vec()
        };
...
/// Builder for .snd archives. Use this to pack individual WAV files into a
/// single game-compatible sound library.
#[derive(Default)]
...
impl SndWriter {
    ...
    /// Add a sound file to the archive. `name` should be the filename without
    /// extension, max 39 characters. `data` is the binary sound data.
    /// `decompressed_size` is the original entries metadata value (0 if raw/uncompressed).
    pub fn add(&mut self, name: &str, data: Vec<u8>, decompressed_size: u32) {
    ...
    pub fn save<P: AsRef<Path>>(&self, path: P) -> Result<(), Box<dyn Error>> {
        ...
        let mut offset = 4 + count * (40 + 4 + 4 + 4);
        ...
        for (name, data, decomp_size) in &self.entries {
            ...
            cursor.write_u32::<LittleEndian>(*decomp_size)?;
...
/// Encode one PCM sample into an IMA-ADPCM nibble, updating predictor and step index in place.
fn encode_sample(sample: i16, predictor: &mut i32, step_index: &mut i32) -> u8 {
    let step = STEP_TABLE[*step_index as usize];
    ...
    // Quantize the difference into a 3-bit magnitude
    if abs_diff >= step {
        nibble |= 4;
        abs_diff -= step;
    }
    ...
    // Update predictor and step index using the decoder's exact logic
    // (This ensures encoder and decoder stay in sync)
    decode_nibble(nibble, predictor, step_index);
