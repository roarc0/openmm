...
pub fn spawn_actors_with_cache(
    ...
    for (i, actor) in prepared.actors.iter().enumerate() {
        ...
        let (standing_frames, sprite_w, sprite_h) =
            sprites::load_sprite_frames(st_root, lod_manager, images, materials, &mut Some(cache), 0, 0, 0);
        if standing_frames.is_empty() || sprite_w == 0.0 {
            continue;
        }
        let (walking_frames, _, _) =
            sprites::load_sprite_frames(wa_root, lod_manager, images, materials, &mut Some(cache), 0, 0, 0);
...
pub fn spawn_monsters_with_cache(
    ...
    for monster in &prepared.monsters {
        ...
        for (st, wa) in &sprite_fallbacks {
            let (sf, w, h) = sprites::load_sprite_frames(st, lod_manager, images, materials, &mut Some(cache), monster.variant, 0, 0);
            if !sf.is_empty() && w > 0.0 {
                standing_frames = sf;
                sprite_w = w;
                sprite_h = h;
                let (wf, _, _) = sprites::load_sprite_frames(wa, lod_manager, images, materials, &mut Some(cache), monster.variant, 0, 0);
                walking_frames = wf;
                break;
            }
        }
